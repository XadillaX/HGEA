...
void CALL HGE_Impl::System_SetStateBool(hgeBoolState state, bool value) {
    switch (state) {
        case HGE_WINDOWED: bWindowed = value; break;
        case HGE_ZBUFFER: bZBuffer = value; break;
        case HGE_TEXTUREFILTER: bTextureFilter = value; break;
        case HGE_USESOUND: bUseSound = value; break;
...
void CALL HGE_Impl::System_SetStateHwnd(hgeHwndState state, HGE_HWND value) {
    switch (state) {
        case HGE_HWNDPARENT: hwndParent = value; break;
        default: break;
    }
}
...
void CALL HGE_Impl::System_SetStateInt(hgeIntState state, int value) {
    switch (state) {
        case HGE_SCREENWIDTH: nScreenWidth = value; break;
        case HGE_SCREENHEIGHT: nScreenHeight = value; break;
        case HGE_SCREENBPP: nScreenBPP = value; break;
        case HGE_SAMPLERATE: nSampleRate = value; break;
        case HGE_FXVOLUME: nFXVolume = value; break;
        case HGE_MUSVOLUME: nMusVolume = value; break;
        case HGE_STREAMVOLUME: nStreamVolume = value; break;
        case HGE_FPS: nHGEFPS = value; break;
        default: break;
    }
}
...
void CALL HGE_Impl::System_SetStateString(hgeStringState state, const char *value) {
    switch (state) {
        case HGE_ICON: szIcon = value; break;
        case HGE_TITLE: 
            if (value) strncpy(szWinTitle, value, sizeof(szWinTitle) - 1); 
            break;
        case HGE_INIFILE: 
            if (value) strncpy(szIniFile, value, sizeof(szIniFile) - 1); 
            else szIniFile[0] = 0;
            break;
        case HGE_LOGFILE: 
            if (value) strncpy(szLogFile, value, sizeof(szLogFile) - 1); 
            else szLogFile[0] = 0;
            break;
        default: break;
    }
...
hgeCallback CALL HGE_Impl::System_GetStateFunc(hgeFuncState state) {
    switch (state) {
        ...
        case HGE_GFXRESTOREFUNC: return procGfxRestoreFunc;
...
const char* CALL HGE_Impl::System_GetStateString(hgeStringState state) {
    switch (state) {
        ...
        case HGE_INIFILE: return szIniFile[0] ? szIniFile : nullptr;
        case HGE_LOGFILE: return szLogFile[0] ? szLogFile : nullptr;
        default: return nullptr;
    }
}
